...
def play_time():
    try:
        ...
        coverted_current_time = time.strftime('%M:%S' , time.gmtime(current_time))
        ...
        # Chhange the status bar
        status_bar_lbl.configure(text=f"{coverted_current_time} -/- {current_song_length}")

        # Update the Song Slider according to current song timer/position
        song_slider.configure(value=int(current_time)) 
        # Update time
        slider_label['text'] = song_slider.get()
